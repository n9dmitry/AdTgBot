...
class CarBotHandler:
    ...
    async def handle_photos(self, message, state, user_id):
        ...
        await self.send_preview(message, state)
        await state.finish()  # Finish the state

    async def send_preview(self, message, state):
    ...
    async def send_photos_and_clear_buffer(self, message, state):
        await self.send_preview(message, state)
...
async def send_advertisement(message: types.Message, state: FSMContext):
    await car_bot.send_preview(message,state)
