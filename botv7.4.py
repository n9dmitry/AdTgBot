...
class CarBotHandler:
    ...
    async def handle_photos(self, message, state):
        ...
        # Send the message with the photo added and the keyboard
        await message.reply("Фото добавлено")

        # Finish the state
        await state.finish()

        # Call the send_advertisement method to send the advertisement
        await self.send_advertisement(message, state)
...
# Create an instance of the class and use the methods in your code
    ...
    #
        ...
        # )
    #
    #     # Создаем фотографию для предпросмотра
    #     # Создаем фотографию для предпросмотра
    #     photo_preview = InputMediaPhoto(media=photo_id, caption="Предпросмотр")
    #
    #     # Создаем сообщение с предпросмотром заголовка
    #     caption_preview = f"<b>Предпросмотр:</b>\n\n{caption}"
    #
    #     # Отправляем фото и предпросмотр заголовка пользователю
    #     await bot.send_photo(chat_id=message.chat.id, photo=photo_id, caption=caption_preview,
    #                          parse_mode=types.ParseMode.HTML)
    #     await bot.send_photo(chat_id=message.chat.id, photo=photo_preview)
    #
    #     # Сохраняем информацию о фото и заголовке
    ...
    #             # Отправляем фото и заголовок в канал
    ...
    #
    #             # Очищаем информацию о фото и заголовке
    #             del user_data["photo_id"]
    #             del user_data["caption"]
    #
    #             await bot.send_message(user_id, "Фотография и заголовок отправлены в канал.")
...
async def send_advertisement(message: types.Message, state: FSMContext):
    await car_bot.send_photos_to_preview(message, state)
