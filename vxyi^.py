...
class CarBotHandler:
    ...
    # async def send_photos_to_preview(self, message, state):
    #     user_id = message.from_user.id
    #     async with lock:
    #         if buffered_photos:
    #             await bot.send_media_group(chat_id=user_id, media=buffered_photos, disable_notification=True)
    ...
    #             # )
    #                 await bot.send_media_group(chat_id=CHANNEL_ID, media=buffered_photos, disable_notification=True)
    #             # await bot.send_message(user_id, "Фотографии отправлены в канал.", reply_markup=keyboard)
    #                 buffered_photos.clear()
...
async def handle_photos_handler(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    await car_bot.handle_photos(message, state, user_id)
